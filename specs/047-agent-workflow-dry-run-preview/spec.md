...
**Input**: User description: "Create Spec 047 production-code E2E smoke test for the role-based agent orchestration workflow. Add a small CLI dry-run/preview improvement to tools/agent-workflow, with focused tests."

## User Scenarios & Testing

### User Story 1 - Preview the Next Workflow Stage Safely (Priority: P1)

An AIverse maintainer can preview what `tools/agent-workflow` would run for the current workflow stage before spawning the Implementer, Reviewer, or any other configured runner.

**Why this priority**: This is the smallest production-code tooling change that exercises the real Implementer -> Reviewer workflow while preserving safety gates and avoiding product app changes.
...
2. **Given** a workflow state at a review stage configured for the default Reviewer, **When** dry-run preview runs, **Then** the CLI shows that the Reviewer would receive a prompt argument preview without running the Reviewer.
...
- **FR-001**: The workflow CLI MUST provide a dry-run preview for the `run` command that does not spawn the Implementer, Reviewer, or any configured agent process.
...
- **FR-008**: Automated tests MUST use fake or injected adapters and MUST NOT call live CLI agents.
...
- **SC-005**: The full validation suite passes before the production-code smoke task is considered ready for live Implementer -> Reviewer orchestration.
...
- The live Implementer -> Reviewer run for this feature will be manually triggered after the spec, plan, and tasks are reviewed.
