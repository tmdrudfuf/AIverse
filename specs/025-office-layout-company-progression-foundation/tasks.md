...
- [ ] T048 Add snapshot stability tests for context summaries, timestamp fallback/context timestamps, sorted updateMany()/getSnapshots() output, and clone immutability in `src/features/city-view/scene/office/employees/EmployeeAIService.test.ts`.
- [ ] T049 Add controller integration tests for getEmployeeAIStateSnapshots() read-only composition of simulation, workstation, schedule, movement, progression, and layout context in `src/features/city-view/scene/office/OfficeProjectPortalController.employee-ai.test.ts`.
