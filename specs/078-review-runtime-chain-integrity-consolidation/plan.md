...
**Superseded** (combined branch, round 2 review, P1-001): self-consistency alone let a malformed target change `baseSha` and `mergeBaseSha` together and still pass, without ever matching the approved comparison base. The fix does exactly what this decision argued against inventing — `ReviewTarget.ts` now exports `REVIEW_TARGET_BASE_BRANCH`, `computeReviewTargetBaseSha(projectId, planId)`, and `computeReviewTargetSha(implementerRuntimeId)` as the one canonical formula both `resolveReviewTarget` and the shared validator call, closing the gap this decision's "no invented fields" reasoning had left open. See `specs/077-078-combined-review-note.md` for the finding and fix.
