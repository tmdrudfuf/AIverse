...
1. `validateReviewRuntimeChainIntegrity` closes all nine audited identity/rules-version gaps (`.agent-workflow/spec-078-chain-integrity-audit.md`), each recomputing a record's id via that record's own canonical `create*Id` helper and checking its `rulesVersion` against that record's own canonical constant — no invented field, no duplicated id-formula string.
