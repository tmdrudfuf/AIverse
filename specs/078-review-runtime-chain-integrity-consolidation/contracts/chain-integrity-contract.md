...
Checks 4 and 5 are independently applied — a malformed id with a valid `rulesVersion`, and a valid id with an unsupported `rulesVersion`, are each caught on their own; neither can compensate for the other passing.
