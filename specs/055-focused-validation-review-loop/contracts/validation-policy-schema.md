...
Inspects `state.validationRuns`/`state.reviewRuns` for the latest `phase: "full"` batch and the latest Approved review; `satisfied` is `true` when the full batch's last command status is `"passed"`, the latest review's outcome is `"Approved"`, and the two targets do not **positively disagree**. A record with no `target` at all (legacy/pre-Spec-055 data, or state built without target tracking) is "unknown", not a mismatch — Spec 054 already reported `humanGate.ready: true` with no commit-match evidence whenever every other condition held, and this feature must not retroactively withdraw that for legacy data. Only when **both** targets are present and `targetsMatch(...)` is `false` does `satisfied` become `false` (`reason: "target-mismatch"`). Used identically by `runSummary.js`'s `humanGate`/`finalReadinessSatisfied` computation and by any future readiness check, so the two can never disagree (Architecture Decision: single source of truth for this question).
