...
- Q: How does `--skip-validation` interact with readiness? → A: Unchanged from Spec 054/existing behavior, and reconfirmed here: `--skip-validation` skips every validation stage occurrence in that invocation (focused and full alike) and marks `validationSkipped: true`; `humanGate.ready` already requires `validation.status === "passed"` and skipped validation is reported as `"skipped"`, never `"passed"`, so a skipped run can never become merge-ready. This feature does not add a way to skip only the full phase while keeping readiness.
...
- **FR-012**: `--skip-validation` MUST continue to skip every validation occurrence (focused and full alike) in that invocation and MUST continue to make `humanGate.ready` unreachable for that invocation (validation status `"skipped"`, never `"passed"`).
