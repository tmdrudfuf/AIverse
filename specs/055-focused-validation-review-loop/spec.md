...
- Q: What happens if either the reviewed target or the full-validation target is missing entirely (no `target` field at all)? → A: `isFinalValidationSatisfied` requires **both** targets to be present; missing evidence on either side is `satisfied: false` (`reason: "target-evidence-missing"`), matching FR-010 ("readiness MUST be false whenever this comparison ... is inconclusive, or either target is missing") exactly — not treated as a free pass. An earlier draft of this feature treated missing evidence as "unknown, not disproven" for backward compatibility with pre-Spec-055 data, which directly contradicted FR-010 as written (Codex review round 2, finding P1-001); that permissiveness has been removed. Old run directories remain fully *readable* (FR-023) — they simply report `finalReadinessSatisfied: false`/`exactCommitMatch: "unknown"` rather than a confident `true`, since Spec 055 introduces exact-match evidence as a new readiness requirement that did not exist for them.
...
- **FR-010**: The workflow MUST expose a deterministic exact-match comparison between the latest Approved review's `target` and the latest `final-verification` attempt's `target`; readiness MUST be `false` whenever this comparison fails, is inconclusive, or either target is missing, and `commits.exactCommitMatch` MUST report `true`/`false` only when both targets are present, `"unknown"` otherwise (never fabricated). This comparison MUST be consulted at the orchestration-decision level too, not only by the run summary — `runOrchestration` MUST NOT call `markHumanGate` when `isFinalValidationSatisfied` reports `satisfied: false`, even if `final-verification`'s own commands passed and its tree-diff check saw no change during its own execution, since a resumed run can supply an Approved review record that predates target tracking (Codex review round 3, finding P1-001).
...
- **FR-013**: Unsafe-command rejection (`assertSafeValidationCommand`, remote-mutation rejection) MUST apply identically to focused and full command lists, before any subprocess spawns, with no separate or weaker code path for either. For a multi-command list, every command MUST be parsed and safety-checked before any command in that same list spawns — an earlier safe command MUST NOT execute while a later command in the same list has not yet been checked (Codex review round 4, finding P1-001; the original per-command parse-check-spawn loop violated this for every command after the first).
