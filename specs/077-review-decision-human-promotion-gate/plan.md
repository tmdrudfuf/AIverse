...
Before deriving a classification or accepting a Promote action, `ReviewDecisionService` re-derives and revalidates the same chain `ReviewerRuntimeService.validateContext` already validates (Execution Plan through Reviewer Runtime), via its own private `validateChain` function with `REVIEW_PROMOTION_*`-prefixed reason codes. **Correction made during implementation:** the repository's established convention is that each pipeline stage duplicates its own validation logic rather than importing a sibling stage's private functions — confirmed by `ImplementerRuntimeService.ts` and `ReviewerRuntimeService.ts` each already maintaining their own independent, non-exported `validateContext`, and `getActorBlockReason` being independently duplicated across five existing services. `ReviewDecisionService.validateChain` follows this same precedent: it mirrors `ReviewerRuntimeService.validateContext`'s field-by-field checks rather than importing or exporting them, preserving the Constitution Check's "Reviewer Runtime services reused unmodified" (zero edits to `reviewer-runtime/*.ts`). Following a round-5 review finding and a subsequent one-time comprehensive field-by-field parity audit (see `review.md`, "Comprehensive parity audit"; working matrix in `.agent-workflow/spec-077-parity-audit.md`), `validateChain` now mirrors every applicable field-level check `ReviewerRuntimeService.validateContext`/`validateRoleBinding`/`validateReviewTarget` perform against Runtime Start, Implementer Runtime, and Review Target — including their `worktreePath`/`branch`/`specificationPath` linkage and the Review Target's `workingTreeState === "Clean"` guard — not merely the subset it originally covered. It adds checks beyond that full mirror that have no `validateContext`/`validateReviewTarget` equivalent because Spec 076 produces the Reviewer Runtime Result rather than later re-trusting it: the Reviewer Runtime's own `status === "Completed"`/`decision === "Approved"`, the paired Reviewer Runtime Result's status/decision parity against it, and — following a round-6 review finding (see `review.md`, "Round 6") — the Reviewer Runtime Result's own `runtimeStartId`/`implementerRuntimeId` parity against the current Runtime Start/Implementer Runtime, mirroring the equivalent check the Implementer Runtime Result side already had since the round-1 audit. Per `docs/agent-workflow/token-efficient-review-policy.md`, this feature's revalidation depth matches its closest sibling exactly, rather than attempting a repository-wide linkage audit beyond what that sibling performs.
