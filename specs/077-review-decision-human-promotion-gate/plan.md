...
Before deriving a classification or accepting a Promote action, `ReviewDecisionService` re-derives and revalidates the same chain `ReviewerRuntimeService.validateContext` already validates (Execution Plan through Reviewer Runtime), reusing those existing validation functions rather than duplicating their logic. It adds exactly one further check beyond what Spec 076 validates: the Reviewer Runtime's own `status === "Completed"` and `decision === "Approved"`. Per `docs/agent-workflow/token-efficient-review-policy.md`, this feature's revalidation depth intentionally matches its closest sibling (`ReviewerRuntimeService.validateContext`) and does not attempt a deeper repository-wide linkage audit beyond what that approved, already-reviewed sibling performs.
