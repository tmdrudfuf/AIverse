...
3. `review.finalDecision` is never `"Approved"` when `review.structuredReviewStatus === "invalid"`. (`"absent"` is a legitimate Markdown-only Approved decision — Spec 050 compatibility — not a reason to withhold approval; only genuinely malformed structured data blocks it.)
4. `run.stopReason` is `null` whenever `run.status` is `"awaiting-human-decision"` or `"planned"` (nothing has stopped in either case); it is populated for every other status.
