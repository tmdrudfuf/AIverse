...
- To pick the Implementer for one execution without editing the state file, pass `--implementer <agent-id>` to `orchestrate`, `run`, `run-review`, or `detect-agent` (e.g. `--implementer claude`); the workflow automatically resolves the other configured agent as Reviewer. Priority is CLI override, then state-configured roles, then defaults. The override never rewrites the state file, and a resumed `orchestrate` run keeps its originally resolved roles regardless of a missing or matching override, rejecting a conflicting one before any spawn. See `tools/agent-workflow/README.md#runtime-role-selection`.
...
shell commands, and other important information, read specs/085-approved-re-review-promotion-eligibility-foundation/plan.md
<!-- SPECKIT END -->
